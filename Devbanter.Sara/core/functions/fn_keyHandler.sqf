...
switch (_code) do
{
	...
	// O, police gate opener || civilian ziptie action
    if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget in [civilian,west]) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1 && (animationState cursorTarget == "Incapacitated" || cursorTarget getvariable ["surrender", false])) then
  		{
