...
[] spawn
{
	...
	while{true} do
	{
		...
		_load = round(_cfg / 8);
		if (backpack player == "B_AssaultPack_cbr") then { _load = 30; };
		if (backpack player == "B_Kitbag_mcamo") then { _load = 40; };
		if (backpack player == "B_TacticalPack_oli") then { _load = 50; };
		if (backpack player == "B_FieldPack_ocamo") then { _load = 60; };
		if (backpack player == "B_Bergen_sgg") then { _load = 70; };
		if (backpack player == "B_Kitbag_cbr") then { _load = 80; };
		if (backpack player == "B_Carryall_oli") then { _load = 100; };
		if (backpack player == "B_Carryall_khk") then { _load = 100; };
