...
if(!(EQUAL(_level,-1))) then {
    _flag = switch(playerSide) do {case west: {(_level <= (FETCH_CONST(life_copLevel)))}; case independent: {(_level <= (FETCH_CONST(life_medicLevel)))}; case east: {(_level <= (FETCH_CONST(life_adacLevel)))}; default {true};};
