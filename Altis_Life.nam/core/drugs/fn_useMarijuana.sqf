...
playSound3D ["A3VL_sounds\sounds\bong.ogg", player, false, getPosASL player, 1, 1, 45];
...
while{_num > 0} do {
	...
	_num = _num - 1;
	if(_num == 4) then {
		["Your high is wearing off in around 30 seconds!", false] spawn domsg;
	};
	if(_num == 2) then {
		["Your high is wearing off in around 15 seconds!", false] spawn domsg;
	};
};
