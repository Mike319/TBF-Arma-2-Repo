// all kinds of goodies to stop haxxxx in here.... 
...
AHAH = compileFinal "
	...
	[] spawn {
		...
		diag_log ""**ShutEmDown Antihack**:Detection of hack variables started!"";
		while {
			true
		}
		do {
			{
				if (!(isNil _x) && !(_uid in _list)) exitWith {
					diag_log ""**ShutEmDown Antihack**: Found a hack variable!"";
	...
	[] spawn {
		...
		diag_log ""**ShutEmDown Antihack**: Hack menu check started!"";
		while {
			true
		}
		do {
			...
			if (!(isNull findDisplay 3030) && !(_uid in _list)) then {
					...
					diag_log ""**ShutEmDown Antihack**:Found a hack menu!"";
...
diag_log "**ShutEmDown Antihack**: All Checks Started";
diag_log "**ShutEmDown Antihack**: Adding PublicVariableEventHandler";
hackerLog = [];
"hackFlag" addPublicVariableEventHandler
{
    ...
    diag_log format["**ShutEmDown Antihack**:  %1   Player ID: (%2) was detected for %3 with the value '%4'", name _player, getPlayerUID _player, _hackType, _hackValue];
    hint format["**ShutEmDown Antihack**:  %1 (Player ID: %2) was detected for %3 with the value '%4'", name _player, getPlayerUID _player, _hackType, _hackValue];
...
addPublicVariableEventHandler {
    ...
    diag_log format["**ShutEmDown Antihack**: running on %1", owner _client];
};			

antihack_debug = true;
diag_log "**ShutEmDown Antihack **: Misc scanning started.";